...
func LoadServer(cb func(conn connect.Connection)) *Server {
	// Create Server
	net, identity, identityConnect := newServer(false)
	...
	// Initialize the CONNECT server
	connectServer, err := connect.StartServer(
		identityConnect, cb, net, params, connect.DefaultConnectionListParams(),
	)
	if err != nil {
		jww.FATAL.Panicf("[%s] Unable to start connection server: %+v", logPrefix, err)
	}
	jww.INFO.Printf("[%s] Initialized CONNECT Server", logPrefix)
...
func newServer(initialize bool) (*xxdk.Cmix, xxdk.ReceptionIdentity, xxdk.ReceptionIdentity) {
	...
	// Get the identity for connect server
	identityStorageKey = "identityStorageKeyConnect"
	identityConnect, err := xxdk.LoadReceptionIdentity(identityStorageKey, net)
	if err != nil {
		if initialize {
			// If no extant xxdk.ReceptionIdentity, generate and store a new one
			identityConnect, err = xxdk.MakeReceptionIdentity(net)
			if err != nil {
				jww.FATAL.Panicf("[%s] Failed to generate reception identity: %+v", logPrefix, err)
			}
			err = xxdk.StoreReceptionIdentity(identityStorageKey, identityConnect, net)
			if err != nil {
				jww.FATAL.Panicf("[%s] Failed to store new reception identity: %+v", logPrefix, err)
			}
		} else {
	...
	if initialize {
		...
		err = utils.WriteFileDef(outputFile+".connect", identityConnect.GetContact().Marshal())
		if err != nil {
			jww.FATAL.Panicf("[%s] Failed writing contact file to %v: %+v", logPrefix, outputFile+".connect", err)
		}
	}

	return net, identity, identityConnect
}
