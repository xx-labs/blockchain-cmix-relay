...
// Connect server contact file
var contactFileConnect string
...
var rootCmd = &cobra.Command{
	Use:   "client",
	Short: "Runs a HTTP cMix relay client",
	Long:  `Client provides an HTTP server that proxies any HTTP requests over cMix`,
	Args:  cobra.NoArgs,
	Run: func(cmd *cobra.Command, args []string) {
		...
		cmix := api.NewClient(config)

		// Start cmix
		cmix.Start()

		// Create HTTP proxy server
		server := NewHttpProxy(cmix, port, contactFile, contactFileConnect, logPrefix)
...
func init() {
	...
	// Contact files
	rootCmd.Flags().StringVarP(&contactFile, "contactFile", "c", "http.xxc", "Path to file containing the REST server contact info")
	rootCmd.Flags().StringVarP(&contactFileConnect, "contactFileConnect", "o", "http-connect.xxc", "Path to file containing the CONNECT server contact info")
	// Retries
